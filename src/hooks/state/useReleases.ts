...
function loadSubscriptions(): ReleaseSubscription[] {
  try {
    const stored = localStorage.getItem(STORAGE_KEYS.releaseSubscriptions);
    if (stored) {
...
export function useReleases() {
  const [subscriptions, setSubscriptions] = useState<ReleaseSubscription[]>(
    () => loadSubscriptions(),
  );
  ...
  const addSubscription = useCallback(
    (repoFullName: string, repoId?: number) => {
      setSubscriptions((prev) => {
        // Check if already subscribed
        if (
          prev.some(
            (sub) =>
              sub.repoFullName.toLowerCase() === repoFullName.toLowerCase(),
          )
        ) {
          return prev;
        }
  ...
  const addSubscriptions = useCallback(
    (repos: { fullName: string; id?: number }[]) => {
      setSubscriptions((prev) => {
        const existing = new Set(
          prev.map((sub) => sub.repoFullName.toLowerCase()),
        );
        const newSubs = repos
  ...
  const removeSubscription = useCallback((repoFullName: string) => {
    setSubscriptions((prev) =>
      prev.filter(
        (sub) => sub.repoFullName.toLowerCase() !== repoFullName.toLowerCase(),
      ),
    );
  }, []);
  ...
  const updateLatestRelease = useCallback(
    (repoFullName: string, release: ReleaseSubscription["latestRelease"]) => {
      setSubscriptions((prev) =>
        prev.map((sub) => {
