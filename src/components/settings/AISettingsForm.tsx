...
import { Loader2, CheckCircle, XCircle, Eye, EyeOff, RotateCcw } from 'lucide-react';
import { AISettings, AIProviderConfig, AnalysisDepth } from '@/hooks/useAISettings';
...
export function AISettingsForm({ settings, onUpdate, onUpdateProvider }: AISettingsFormProps) {
  ...
  const handleTestConnection = async () => {
    ...
    try {
      const data = await postJson<{ success: boolean; error?: string }>('/api/test-ai-connection', {
  ...
  const isProviderConfigured = settings.provider.baseUrl && settings.provider.apiKey;
  const defaultSystemPrompt = getDefaultSystemPrompt(settings.analysisDepth as AnalysisDepth);
      ...
      <CardContent className="space-y-6">
        ...
        {settings.enabled && (
          <>
            ...
            <div id="ai-provider" className="space-y-4 pt-4 border-t scroll-mt-20">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value: 'openai' | 'custom') => onUpdateProvider({ requestFormat: value })}
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="custom">自定义格式（暂不支持）</SelectItem>
                  </SelectContent>
                </Select>
                <p className="text-xs text-muted-foreground">
            ...
            <div id="ai-prompt" className="space-y-6 pt-4 border-t scroll-mt-20">
              <h4 className="font-medium text-sm">提示词配置</h4>
              
              {/* System Prompt */}
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <Label htmlFor="system-prompt">System Prompt（系统提示词）</Label>
                  <Button
                    variant="ghost"
                    size="sm"
            ...
            {/* Analysis Options */}
            <div id="ai-options" className="space-y-4 pt-4 border-t scroll-mt-20">
              ...
              <div className="flex items-center justify-between">
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) => onUpdate({ autoAnalyzeNewStars: checked })}
                />
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) => onUpdate({ suggestLists: checked })}
                />
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) => onUpdate({ autoCreateTags: checked })}
                />
              </div>
            </div>
          </>
        ...
        {settings.lastAnalyzedAt && (
          <div className="pt-4 border-t">
            <p className="text-sm text-muted-foreground">
              上次分析时间：{new Date(settings.lastAnalyzedAt).toLocaleString('zh-CN')}
            </p>
          </div>
        )}
      </CardContent>
