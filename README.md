...
**隐私与安全说明：** 当前采用"纯前端模式"：Token 保存在浏览器本地存储，不会上传到应用后端；GitHub API 请求由浏览器直连 GitHub。

这带来更高的隐私可控性，但也意味着：若站点发生 XSS，恶意脚本理论上可读取本地凭据。请务必使用最小权限 Token，并仅在受信任环境下使用。
...
另外，服务端会对 AI Provider `Base URL` 做基础安全校验（仅允许公网 `https`，拒绝本地/内网地址）。如需限制允许调用来源，请在部署环境配置 `ALLOWED_ORIGINS`（逗号分隔）。

### 安全模型（已锁定）

**决策时间：** 2026-02-09

当前项目采用 **A 方案：纯前端受信终端模式（非 BFF）**。

- GitHub Token 与 AI Key 由用户在前端输入并保存在浏览器本地存储
- GitHub 数据请求由浏览器直连 GitHub API
- AI 请求通过同部署的 `/api/*` 无状态转发到 Provider（仅转发，不持久化密钥）

**接受的风险（已明确）：**

- 若发生同源 XSS，本地存储中的 Token / AI Key 可能被读取
- 无法像 BFF 会话模型那样做服务端集中撤销与细粒度会话治理
- 如未配置 `ALLOWED_ORIGINS`，跨域来源控制将依赖默认策略

**当前不做项（YAGNI）：**

- 不引入服务端托管密钥与 `HttpOnly` 会话（BFF 重构）
- 不实现多租户级权限隔离、审计流水、集中风控

如果你的部署场景需要更高安全等级（如企业多用户、零信任终端），建议演进到 BFF 模式。
